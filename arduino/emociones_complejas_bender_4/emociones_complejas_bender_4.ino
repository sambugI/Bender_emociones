...
void animRainbowStep() {
  ...
  uint32_t ahora = millis();
  if (ahora - rainbow_t0 < RAINBOW_INTERVAL) return;
  rainbow_t0 = ahora;

  // -----------------------------
  //   1. OJOS (tiraIzq / tiraDer)
  // -----------------------------
  for (uint16_t i = 0; i < NUM_LEDS; i++) {
    uint8_t idx = (uint8_t)((i * 256UL / NUM_LEDS) + rainbow_pos);
    //uint32_t c = Wheel(idx);
    uint32_t c = tiraIzq.Color(255, 255, 255);
    tiraIzq.setPixelColor(i, c);
    tiraDer.setPixelColor(i, c);
  }

  // -----------------------------
  //   2. BOCA (matriz completa)
  //     rainbow vertical por filas
  // -----------------------------
  //for (int fila = 0; fila < ROWS; fila++) {

    // Cada fila recibe un color levemente desplazado
  //  uint8_t idx = (rainbow_pos + fila * 20) & 0xFF;
  //  uint32_t c = Wheel(idx);

  //  for (int col = 0; col < COLS; col++) {
  //    strip.setPixelColor(mapLED(fila, col), c);
  //  }
  //}

  // Avanzar animación arcoíris
  rainbow_pos = (rainbow_pos + 1) & 0xFF;

  // Actualizar TODA la cara
  showAll();        // Si esto hace show() en todas tus tiras
  strip.show();     // Asegura que la matriz también se actualice
}
