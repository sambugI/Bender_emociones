...
// ---------- Estados ----------
enum Mode { MODE_OFF, MODE_FELICIDAD, MODE_TRISTEZA, MODE_ENOJO, MODE_SORPRESA, MODE_TRANQUI, MODE_SI, MODE_NO, MODE_TALKING };
...
//modo conversador
unsigned long talking_t0 = 0;
const unsigned long TALKING_INTERVAL = 250; // mouth open/close speed
bool talking_mouth_open = false;
...
void setup() {
  ...
  Serial.println("\nComandos: felicidad | tristeza | enojo | sorpresa | off | tranqui | si | no | talking ");
}
...
void leerComando() {
  ...
  else if (cmd == "talking")     modoActual = MODE_TALKING;
...
void actualizarAnimacion() {
  ...
  if (modoActual != lastMode || forceRedraw) {
    ...
    switch (modoActual) {
      ...
      case MODE_TALKING:
        moverceja(85);
        dibujarModoUnaVez(tiraIzq.Color(100, 200, 255)); // Light blue eyes
        smoothMoveOrejas(120, 120, 6);
        dibujarTalkingInit();
        showAll();
        break;
    }
  ...
  if (modoActual == MODE_TALKING)   animTalkingStep();
}
...
void dibujarTalkingInit() {
  // Initial mouth position (closed)
  uint32_t cyan = strip.Color(0, 200, 255);
  
  // Horizontal line (mouth closed)
  for (int c = 4; c <= 11; c++) {
    strip.setPixelColor(mapLED(2, c), cyan);
    strip.setPixelColor(mapLED(3, c), cyan);
  }
}

void animTalkingStep() {
  unsigned long ahora = millis();
  if (ahora - talking_t0 < TALKING_INTERVAL) return;
  talking_t0 = ahora;
  
  talking_mouth_open = !talking_mouth_open;
  
  // Clear mouth area
  for (int r = 0; r < ROWS; r++) {
    for (int c = 0; c < COLS; c++) {
      strip.setPixelColor(mapLED(r, c), 0);
    }
  }
  
  uint32_t cyan = strip.Color(0, 200, 255);
  
  if (talking_mouth_open) {
    // Mouth OPEN - oval shape
    // Row 1 - top
    for (int c = 6; c <= 9; c++)
      strip.setPixelColor(mapLED(1, c), cyan);
    
    // Row 2 - sides
    strip.setPixelColor(mapLED(2, 5), cyan);
    strip.setPixelColor(mapLED(2, 10), cyan);
    
    // Row 3 - sides
    strip.setPixelColor(mapLED(3, 5), cyan);
    strip.setPixelColor(mapLED(3, 10), cyan);
    
    // Row 4 - bottom
    for (int c = 6; c <= 9; c++)
      strip.setPixelColor(mapLED(4, c), cyan);
      
  } else {
    // Mouth CLOSED - horizontal line
    for (int c = 4; c <= 11; c++) {
      strip.setPixelColor(mapLED(2, c), cyan);
      strip.setPixelColor(mapLED(3, c), cyan);
    }
...
void animTRANQUIStep() {
  unsigned long ahora = millis();
  if (ahora - tranqui_t0 < TRANQUI_INTERVAL) return;
  tranqui_t0 = ahora;
...
void animTRANQUIReset() {
  // Soft warm white for calm feeling
  uint32_t blanco_calido = Adafruit_NeoPixel::Color(180, 160, 140);

  // --- OJOS: soft warm white ---
  for (int i = 0; i < NUM_LEDS; i++) {
    tiraIzq.setPixelColor(i, blanco_calido);
    tiraDer.setPixelColor(i, blanco_calido);
  }
  tiraIzq.show();
  tiraDer.show();
  ...
  // Draw a soft smile
  // Bottom row (row 5) - base of smile
  for (int c = 4; c <= 11; c++)
    strip.setPixelColor(mapLED(4, c), blanco_calido);


  // Row 3 - corners of smile
  strip.setPixelColor(mapLED(3, 3), blanco_calido);
  strip.setPixelColor(mapLED(3, 12), blanco_calido);

  // Optional: add subtle dimples
  strip.setPixelColor(mapLED(3, 2), blanco_calido);
  strip.setPixelColor(mapLED(3, 13), blanco_calido);
...
void dibujarNO() {
  ...
  // Fila 3
  strip.setPixelColor(mapLED(2, 3), rojo);
  strip.setPixelColor(mapLED(2, 12), rojo);
  strip.setPixelColor(mapLED(2, 4), rojo);
  strip.setPixelColor(mapLED(2, 11), rojo);

  // Fila 2 → sonrisa (de 6 a 9)
  for (int c = 5; c <= 10; c++)
    strip.setPixelColor(mapLED(1, c), rojo);
}
